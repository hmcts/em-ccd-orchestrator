...
class AutomatedBundlingScenariosTest extends BaseTest {
    ...
    @Test
    void testBundleWithDocumentOutlineSubtitlesOn() throws IOException {
        String json = TestUtil.readFile(SRC_AAT_RESOURCES_DOCUMENTS_CASE_JSON_FILE_PATH);
        json = json.replace(CONFIGURATION_FILE, "testbundleconfiguration/f-tests-15-document-outline-subtitles.yaml");
        json = findDocumentUrl(json);
        ...
        pollResponse
            .assertThat().log().all()
            .statusCode(200)
            .body(BUNDLE_BUNDLE_TITLE, equalTo("Functional tests bundle 15"))
            .body(BUNDLE_STITCHED_DOCUMENT_URI, notNullValue());
    }

    @Test
    void testBundleWithTableOfContentsSubtitlesOn() throws IOException {
        String json = TestUtil.readFile(SRC_AAT_RESOURCES_DOCUMENTS_CASE_JSON_FILE_PATH);
        json = json.replace(CONFIGURATION_FILE, "testbundleconfiguration/f-tests-16-toc-subtitles.yaml");
        json = findDocumentUrl(json);
        ...
        pollResponse
            .assertThat().log().all()
            .statusCode(200)
            .body(BUNDLE_BUNDLE_TITLE, equalTo("Functional tests bundle 16"))
            .body(BUNDLE_STITCHED_DOCUMENT_URI, notNullValue());
    }
